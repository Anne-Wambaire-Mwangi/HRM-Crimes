...
> **To my team:** Whether you're picking this up for the first time or returning after months away — I've written this so you can understand, reproduce, and extend everything I built. You shouldn't need to ask me a single question to get up to speed.
...
- Crime is growing month over month, but Halifax is still a city — not a warzone. **Stay informed, not afraid.** I built this project partly for people like me who just wanted to know the real picture.
...
- The best documentation is the kind you'd want to find if you were joining someone else's project. Write that.
...
- **Move Calendar to SQL** — My DAX Calendar table works, but a proper `dim.Date` SQL table would be more portable and performant
...
- **Better severity scoring** — My current severity classification is binary (High/Other). A multi-tier model trained on classification codes would be more useful
- **Predictive modelling** — Move from trend forecasting to ARIMA or Prophet as I accumulate more months of data
- **Publish to Power BI Service** — So anyone can view the report in a browser without needing Power BI Desktop installed
...
This project uses publicly available data from the [HRM Open Data Hub](https://catalogue.hrm.opendata.arcgis.com/). All analysis, code, and documentation are my own original work.
